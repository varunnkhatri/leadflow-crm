...
export default async function PipelinePage() {
  ...
  const rows = (data ?? []).map((lead) => { const customer = lead.customers as unknown as { full_name: string | null } | { full_name: string | null }[] | null; const name = Array.isArray(customer) ? (customer[0]?.full_name || "Unknown customer") : (customer?.full_name || "Unknown customer"); return { id: lead.id as string, name, enquiry: lead.enquiry as string | null, stage: String(lead.stage || "NEW"), score: Number(lead.lead_score) || 0, temperature: lead.temperature as string | null, value: Number(lead.deal_value) || 0, updated: String(lead.updated_at || "") }; });
